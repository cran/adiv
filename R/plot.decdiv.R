plot.decdiv <- function(x, ncom = 1, col = "black", csize = 1, legend = TRUE, ...){


if(!ncom%in%(1:ncol(x))) stop("ncom must be the number of the column of x that must be plotted")

if(!inherits(x, "decdiv")) stop("x must be of class decdiv")
...
radi <- sq*he
symbols(XX, YY, circles=radi, bg=col, fg="black", inches=FALSE, add=TRUE)
if(legend==TRUE){
         ...
         for (i in (1:(length(br0)))) {
            ...
            z0 <- br0[i]
            x0 <- x0 + xh + z0
            symbols(x0, y0, circles = z0, bg = col,                   fg = "black", add =TRUE, inches = FALSE)
            x0 <- x0 + z0
        }
}
}
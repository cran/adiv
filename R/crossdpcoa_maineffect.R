...
# In this space, the principal axes of factor A are defined and species points are projected on these axes

    svdYA <- svd(t(YA)%*%BA%*%YA)
    XoA <- X%*%svdYA$u
    YAoA <- YA%*%svdYA$u
    colnames(XoA) <- colnames(YAoA) <- paste("Axis", 1:ncol(XoA), sep="")

    eig <- svdYA$d[svdYA$d>tol]
    rank <- length(svdYA$d[svdYA$d>tol])
    ...
    res$l1 <- as.data.frame(XoA[, 1:nf])
    res$l2 <- as.data.frame(YAoA[, 1:nf])
    res$eig <- eig
    res$lwX <- epsilonk
    res$lwA <- lambdai
    res$lwB <- muj

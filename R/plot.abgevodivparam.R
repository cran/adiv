   ...
   ylim1 = range(x$div[c("Alpha","Gamma"), ]), ylim2 = NULL, ...)
{
   ...
   if(is.numeric(x)){
       y <- as.vector(x)
       names(y) <- names(x)  
       dotchart(y, xlab = "evodiversity", ...)
   }
   if(is.list(x)){
      if(nrow(x$div)==3){
      par(mfrow=c(2,1))
      plot(x$q, x$div[3, ], type = type, col = col[1], pch = pch[1], ylab="evodiversity", xlab="q", ylim = ylim1, ...)
      ...
      plot(x$q, x$div[2, ], type = type, col = col[3], pch = pch[3], ylab="evodiversity", xlab="q", ylim = ylim2,...)
      if(legend[1]){
      ...
      par(mfrow=c(3,1))
      plot(x$q, x$div[4, ], type = type, col = col[1], pch = pch[1], lty = lty[1], ylab="evodiversity", xlab="q", ylim = ylim1, ...)
      ...
      plot(x$q, x$div[2, ], type = type, col = col[3], pch = pch[3], lty = lty[3], ylab="evodiversity", xlab="q", ylim = ylim2,...)
      if(legend[1]){
         legend(legendposi, legend = rownames(x$div)[2], col = col[3], lty = lty[3], pch = pch[3], ...) 
      }    
      plot(x$q, x$div[3, ], type = type, col = col[4], pch = pch[4], lty = lty[4], ylab="evodissimilarity", xlab="q", ylim = c(0, 1),...)
      if(legend[1]){
